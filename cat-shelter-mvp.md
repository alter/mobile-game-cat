...
Оплаты нет. На экране проигрыша есть кнопка **«поставить ещё полку»** — она
снимает ту беду, которая случилась: затор. Нажатие считается, дальше
показывается заглушка «скоро».

**Полка добавляется целиком, то есть три места, а не одно.** Замерено на 400
партиях по всем 37 уровням, игроком жадным и с 12% случайных ошибок:
...
Одно место не спасает, потому что худший затор — это девять разных видов на
полке: чтобы собрать хоть одну тройку, надо вытащить из кучи ещё по два предмета
каждого вида. Три места дают запас на всю оставшуюся партию, а не разовое
облегчение: при девяти местах для затора достаточно пяти разных видов, при
двенадцати нужно шесть.

Название выбрано не случайно. Полка у нас — три ряда по три, и три места это
**ещё один ряд**. «+3 слота» — язык таблицы; в игре про уют ставят полку.

**Применяется один раз за уровень.** Без этого ограничения доля побед уходит к
ста процентам, условие проигрыша исчезает, а вместе с ним и смысл четвёртой
метрики: платить за спасение станет не от чего.
