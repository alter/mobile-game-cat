...
на три полки по совпадению. Ходы ограничены. Ради чего разбирает — в заброшенном
...
Проигрыш: все девять мест заняты, а совпадений нет. Или кончились ходы.

Ходы: ограничены по уровню (от 25 на первом до 18 на двенадцатом при
том же размере кучи).
...
Оплаты нет. На экране проигрыша есть кнопка «+5 ходов», нажатие считается,
дальше показывается заглушка «скоро».
...
| нажали «+5 ходов» хотя бы раз | > 15% | сопоставимых открытых данных не нашлось |
...
`level_win`, `level_fail`, `moves_button_tap`, `notification_allowed`.
