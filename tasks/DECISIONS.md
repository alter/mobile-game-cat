...
## D18. Строим без проверки спроса — 2026-09-01

**Решение владельца, дословно:** «пока без всяких проверок спроса, просто будем
делать». Ворота 1 (`00-validate-demand/09-go-no-go`) откладываются: рекламный
бюджет не тратится, цена установки не измеряется, и запрет GOAL.md «до ворот 1
не строить оболочку, рисунки, съёмку и магазин» снимается задним числом — он
уже нарушен всей историей репозитория, и это решение делает нарушение
записанным вместо молчаливого.

Что решение НЕ отменяет: ворота 2 (пять посторонних играют) и ворота 3 (живое
испытание с четырьмя метриками) остаются в силе — их никто не отменял, и они
дешевле либо уже оплачены построенным. Задачи `00-validate-demand/08` и `/09`
остаются `todo` с пометкой на это решение: если игра дойдёт до магазина,
вопрос «продаёт ли обещание» встанет снова, и тогда эти задачи готовы.

Цена решения, чтобы она была записана, а не забыта: три недели работы теперь
оправдываются только выходом к воротам 2 и 3, и если они провалятся, ответ
«стоило ли строить» будет куплен дороже, чем предлагал GOAL.md.
